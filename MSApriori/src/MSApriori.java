...
public class MSApriori {
    ...
    private Map<Integer, String> mapWords=null;
    ...
    public void runAlgorithm(String input, String output,
                             double beta, double LS,Map<Integer, String> mapWords) throws IOException {
        ...
        transactionCount=Integer.parseInt(headerLine);
        String headerLine2 = reader.readLine();
        String headerLine3 = reader.readLine();
        String line;
        // for each line (transaction) until the end of the file
        //List<Integer[]> myDatabase = new ArrayList<Integer[]>();
        int j=1;
        ArrayList itemsIn1Tran= new ArrayList<Integer>();
        while (((line = reader.readLine()) != null)) {
            String[] lineSplited = line.split(" ");

            Integer count = mapItemCount.get(Integer.parseInt(lineSplited[1]));
            if (count == null) {
                mapItemCount.put(Integer.parseInt(lineSplited[1]), 1);
                // if this is the largest item ID encountered, then remember it
                if (Integer.parseInt(lineSplited[1]) > maxItemID) {
                    maxItemID = Integer.parseInt(lineSplited[1]);
                }
            } else {
                mapItemCount.put(Integer.parseInt(lineSplited[1]), ++count);
            }

            if(Integer.parseInt(lineSplited[0])==j){
                itemsIn1Tran.add(Integer.parseInt(lineSplited[1]));
            }else{
                if(Integer.parseInt(lineSplited[0])>j){
                    Integer integerArray[] = Arrays.asList(itemsIn1Tran.toArray())
                            .toArray(new Integer[0]);
                    // .toArray(new Integer[objectArray.length]);

                    database.add(integerArray);
                    itemsIn1Tran.clear();
                    itemsIn1Tran.add(Integer.parseInt(lineSplited[1]));
                    j++;
                }
        ...
        System.out.println("::"+transactionCount+"::"+maxItemID);
       // System.out.println("Key Values: "+mapItemCount);
        /*for (int ik=0;ik<database.size();ik++){
            for (int ij=0;ij<database.get(ik).length;ij++){
                System.out.print(" "+ik+":"+ij+": "+database.get(ik)[ij]);
            }
            System.out.println();
        }*/
    ...
    private void saveItemsetToFile(Itemset itemset) throws IOException {
        ...
        try{
            if(itemset!=null){
               String words[] = new String[itemset.size()];
                for (int i=0 ;i<itemset.size();i++){
                    words[i]=mapWords.get(itemset.itemset[i]);
                }
                writer.write(Arrays.toString(words) + " #SUP: "	+ itemset.getAbsoluteSupport());
                writer.newLine();
    ...
    private void saveItemsetToFile(Integer item, Integer support)
        ...
        writer.write(mapWords.get(item) + " #SUP: " + support);
        writer.newLine();
        // increase the number of itemsets found
        itemsetCount++;
    }
